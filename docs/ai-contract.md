...
| `exerciseType` | text | Y | `PUSH_UP` 또는 `SIT_AND_REACH` |
...
`SIT_AND_REACH`의 `metrics`: `{maxTrunkFlexionDeg, minKneeAngleDeg, kneeBentRatio, bounceDetected, holdSec, leftRightAsymmetryDeg}`.
...
| `ExerciseType` (posture 지원 종목) | `PUSH_UP`, `SIT_AND_REACH` |
| `overallLevel` | `EXCELLENT`, `GOOD`, `AVERAGE`, `NEEDS_IMPROVEMENT` |
...
| `AI_UNSUPPORTED_EXERCISE_TYPE` | 400 | PUSH_UP/SIT_AND_REACH 외 종목 | X |
